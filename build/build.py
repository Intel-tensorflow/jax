...
def add_artifact_subcommand_arguments(parser: argparse.ArgumentParser):
  ...
  oneapi_group.add_argument(
      ...
      type=str,
      default="2025.3",
      help="OneAPI version to use. E.g. 2025.3",
  )
...
async def main():
  ...
  if "cuda" in args.wheels and ("rocm" in args.wheels or  "oneapi" in args.wheels):
    logging.error("CUDA and ROCm/OneAPI cannot be enabled at the same time.")
    sys.exit(1)

  if "rocm" in args.wheels and "oneapi" in args.wheels:
    logging.error("ROCm and OneAPI cannot be enabled at the same time.")
    sys.exit(1)
