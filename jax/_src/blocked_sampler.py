...
class SampleFn(Protocol):
  def __call__(self, key: random.KeyArrayLike, shape: Shape,
               *args, **kwargs) -> Array:
    ...
...
def sample_block(
  ...
  size_in_tiles = tuple(
      _shape // _element for _shape, _element in zip(block_size, tile_size))
  def _nested_index(arr, idx):
    if len(idx) == 1:
      return arr[idx[0]]
    return _nested_index(arr[idx[0]], idx[1:])

  def _sample_loop(axis, prefix) -> jax.Array:
    if axis == len(size_in_tiles):
      return sampler_fn(_nested_index(keys,prefix),
                        shape=tile_size, *args, **kwargs)
    else:
      samples = []
